...
def decisionLogic(ego: State):
    ...
    if ego.agent_mode == ThermoMode.Idle and ego.x <= 75:
        output.agent_mode = ThermoMode.Heat
    if  ego.agent_mode == ThermoMode.Heat and ego.x > 85:
...
if __name__ == "__main__":
    ...
    input_code_name = os.path.join(script_dir, "discrete_range.py")
    ...
    init_bruss = [[70, 0, 0, 0], [70, 0, 0, 0]] # setting initial upper bound to 72 causes hyperrectangle to become large fairly quickly
    # -----------------------------------------

    scenario.set_init_single(
        'thermo', init_bruss, (ThermoMode.Idle,)
    )
